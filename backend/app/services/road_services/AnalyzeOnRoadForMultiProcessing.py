...
class AnalyzeOnRoadForMultiprocessing():
    ...
    def __init__(self, regions = settings_metric_transport.REGIONS, path_videos = settings_metric_transport.PATH_VIDEOS,
        ...
        self._agent_frame_get_counter = 0
    ...
    def _signal_handler(self, signum, frame):
        ...
        # Không gọi sys.exit(0) ở đây vì FastAPI/uvicorn sẽ tự xử lý shutdown
        # sys.exit(0) gây lỗi trong asyncio context khi reload
    ...
    def run_multiprocessing(self):
        ...
        for path_video, meter_per_pixel, region in zip(self.path_videos, self.meter_per_pixels, self.regions):
            # Sử dụng os.path để xử lý đường dẫn đúng trên cả Windows và Linux
            import os
    ...
    def get_frame_road(self, road_name : str):
        ...
        # region agent log
        try:
            if self._agent_frame_get_counter % 30 == 0:
                payload = {
                    "sessionId": "debug-session",
                    "runId": "run1",
                    "hypothesisId": "H2",
                    "location": "AnalyzeOnRoadForMultiprocessing.get_frame_road",
                    "message": "frame_encode_sample",
                    "data": {
                        "road_name": road_name,
                        "frame_bytes": len(data) if data else 0,
                        "encode_ms": round(duration_ms, 2),
                    },
                    "timestamp": int(time.time() * 1000),
                }
                with open(LOG_PATH, "a", encoding="utf-8") as _f:
                    _f.write(json.dumps(payload) + "\n")
            self._agent_frame_get_counter += 1
        except Exception:
            pass
        # endregion agent log
        return data
    ...
    def print_all_roads_info(self):
        """In thông tin tất cả roads"""
        print("\n=== ALL ROADS INFO ===")
        for road_name in self.names:
            info = self.shared_data[road_name]['info']
            frame = self.shared_data[road_name]['frame'].get('frame', None)
            print(f"Road: {road_name}")
            print(f"  Info: {dict(info)}")
            print(f"  Frame: {'Có' if frame is not None else 'Không có'}")
        print("=" * 25)
...
if __name__ == '__main__':
    ...
    # In thông tin sau khi khởi động
    import time
    time.sleep(5)  # Đợi process khởi động
    analyzer.print_all_roads_info()
