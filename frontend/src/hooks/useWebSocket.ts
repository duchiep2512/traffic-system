    ...
    try {
      ...
      fetch('http://127.0.0.1:7242/ingest/2c6c6511-3798-41e8-8f0b-7892484d9367', {
        ...
        body: JSON.stringify({
          ...
          runId: 'run1',
          hypothesisId: 'H3',
          location: 'useWebSocket.ts:connect',
          message: 'ws connect attempt',
          data: { wsUrl, hasToken: Boolean(authToken) },
          timestamp: Date.now(),
        }),
      }).catch(() => {});
      // #endregion agent log
...
// Hook specifically for traffic info WebSocket
export const useTrafficInfo = (roadName: string | null) => {
  const wsUrl = roadName ? `${WS_URL}/api/v1/ws/info/${roadName}` : null;
...
// Hook specifically for frame WebSocket
export const useFrameStream = (roadName: string | null) => {
  const wsUrl = roadName ? `${WS_URL}/api/v1/ws/frames/${roadName}` : null;
  const token = getAuthToken();
...
export const useMultipleTrafficInfo = (roadNames: string[]) => {
  ...
  useEffect(() => {
    ...
    roadNames.forEach((road) => {
      ...
      const token = getAuthToken();
      const wsUrl = `${WS_URL}/api/v1/ws/info/${road}`;
...
export const useMultipleFrameStreams = (roadNames: string[]) => {
  ...
  useEffect(() => {
    ...
    roadNames.forEach((road) => {
      if (currentSockets[road]) return;
      const token = getAuthToken();
      const wsUrl = `${WS_URL}/api/v1/ws/frames/${road}`;
      const separator = wsUrl.includes("?") ? "&" : "?";
      const fullUrl = token
        ? `${wsUrl}${separator}token=${encodeURIComponent(token)}`
        : wsUrl;
